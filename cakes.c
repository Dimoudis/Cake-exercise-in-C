...
void gemisma(int alevri[],double wres_zax[][2],int cake){
	...
	int alevri1;
	for (i=0; i<cake; i++){
		printf("Ποσες ωρες ψηθηκε το %iο cake;\n",i+1);
		scanf("%lf",&wres);
		wres_zax[i][0]=wres;
		printf("Ποση ζαχαρη εβαλες στο %iο cake;\n",i+1);
		scanf("%lf",&zaxari);
		wres_zax[i][1]=zaxari;
		printf("Ποσο αλευρι χρησιμοποιησες στο %iο cake;\n",i+1);
		scanf("%i",&alevri1);
		alevri[i]=alevri1;	
	}
	printf("\n");
...
void mo_alevri(int alevri[],int cake){
	...
	printf("O μεσος ορος για το αλευρι ειναι %.3f\n",mo);
}
...
void mo_zaxaris(double wres_zax[][2], int cake){
	...
	mo=sum/cake;
	printf("Ο ΜΟ για την ζαχαρη ειναι %.3f\n",mo);	
}
...
void mo_wrwn_alevri_panw_apo_300(int alevri[],double wres_zax[][2],int cake){
	...
	printf("Ο ΜΟ για τις ωρες για τα cake με αλευρι πανω απο 300 ειναι %.2f\n",mo);
}
...
void max_alevri(int alevri[],double wres_zax[][2],int cake){
	...
	printf("Το μεγιστο αλευρι ειναι %i, το εχει το %io cake. Ψηθηκε %.1f ωρες και εχει %.2f gr ζαχαρης\n",maxi,pos+1,wres_zax[pos][0],wres_zax[pos][1]);
}
...
void max_zaxari(int alevri[],double wres_zax[][2],int cake){
	...
	printf("Η μεγιστη ζαχαρη ειναι %.2f και την εχει το %i cake\n",maxi,pos+1);
	printf("Το αλευρι για το %i cake ειναι %.1f και ψηθηκε σε %.1f ωρες\n",pos+1,alevri[pos],wres_zax[pos][0]);
}
